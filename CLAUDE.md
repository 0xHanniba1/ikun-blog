...
**默认不要自动 commit / push 文章或代码。** 写完只改文件，等用户看完说「提交」/「部署」/「推上线」之类明确字眼，才走 git。

正确顺序：

1. 写文件（md / 代码）
2. **停下，等用户审核**
3. 用户可以本地 `npm run dev` 预览、自己改、或要求修改
4. 用户说「OK 提交」「部署」→ `git add -A` + commit + push
5. 用户说「先别推」/「diff 我看」→ 不动 git，把变更说清楚

例外：用户在同一句话里就说「提交并部署」之类的，可以直接走完。其他情况一律默认「写完不 commit」。
